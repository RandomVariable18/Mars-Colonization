...
function chooseAlgo(e)
{
	...
	if(e.target.innerText=="Take a Tour!")
	{
...
{
	let targetList=[startNode,...endNodeList];
	let visited={};
	let extraList=[];
	for(let i=0;i<targetList.length;i++)
	{
		visited[targetList[i]]=false;
	}
	...
	if(radioHolder[0].children[0].children[0].checked)
	{
		let count=0;
		let SNode=startNode;
		while(count!=targetList.length-1)
		{
			let temp=Dijkstra(visited,extraList,SNode);
			SNode=temp[0];
			extraList=temp[1];
			visited[SNode]=true;
			count++;
	...
	else if(radioHolder[0].children[1].children[0].checked)
	{
		...
		while(count!=targetList.length-1)
		{
			let temp=Dijkstra(visited,extraList,SNode,0);
			SNode=temp[0];
			extraList=temp[1];
			visited[SNode]=true;
			count++;
...
function selectEnd(e)
{
	if(e.target.style.background!="red" && e.ctrlKey )
	{
		...
		if(gridGraph[x][y]=='0')
		{
			...
			gridGraph[x][y]='0';
			endNodeList.push(e.target.className);	
			//console.log(endNode);
		}
	}
...
function Dijkstra(visited,extraList,startN,diagonal=true)
{
	...
	while (!pq.isEmpty()) 
	{
		...
		if(currentNode!=startNode && endNodeList.includes(`${currentNode}`)==false && extraList.includes(currentNode)==false)
		{
	...
	{
		if(!visited[endNodeList[i]])
		{
			if(findMin>times[endNodeList[i]])
			{
				findMin=times[endNodeList[i]];
				endNode=endNodeList[i];
			}
		}
	}
	let path = [endNode];
	let lastStep = endNode;
	while(lastStep !== startN) 
	{
